		...
		case POST_SWEEP_EVENT:
			...
			if(sweepPassed)
				...
				elseif(!cmpstr(opMode, PSQ_DS_SUPRA))
					...
					WAVE spikeDetection = GetLastSettingEachSCI(numericalValues, s.sweepNo, key, s.headstage, UNKNOWN_MODE)
					ASSERT(DimSize(spikeDetection, ROWS) == acquiredSweepsInSet, "Mismatched row count")
					...
					spikeFrequencies[] = spikeDetection[p] / (pulseDuration[p] / 1000)
					...
					try
						...
						if(showPlot)
							if(WindowExists(SPIKE_FREQ_GRAPH))
								...
								ModifyGraph/W=$SPIKE_FREQ_GRAPH mode=3
