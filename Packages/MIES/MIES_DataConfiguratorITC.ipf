...
// whereas the source data generated waves for ITC min sample rate are at 200 kHz
	...
	elseif(dataAcqOrTP == DATA_ACQUISITION_MODE)
		...
		if(onsetDelayUser)
			...
			epochIndexer[] = DC_AddEpoch(panelTitle, DAC[p], epochBegin, epochEnd, "baseline", 0)
		endif
		...
		if(distributedDAQ)
			...
			epochIndexer[] = epochBegin != epochIndexer[p] ? DC_AddEpoch(panelTitle, DAC[p], epochBegin, epochIndexer[p], "baseline", 0) : 0
		...
		if(terminationDelay)
			...
			epochIndexer[] = DC_AddEpoch(panelTitle, DAC[p], epochIndexer[p], epochIndexer[p] + terminationDelay * samplingInterval, "baseline", 0)
		endif
		...
		for(i = 0; i < numEntries; i += 1)
			...
			elseif(config[i][%DAQChannelType] == DAQ_CHANNEL_TYPE_DAQ)
				...
				if(distributedDAQOptOv && offsets[i] > 0)
					...
					DC_AddEpoch(panelTitle, DAC[i], epochBegin, epochBegin + epochOffset, "baseline", 0)
				...
				if(stopCollectionPoint > epochBegin)
					DC_AddEpoch(panelTitle, DAC[i], epochBegin * samplingInterval, stopCollectionPoint * samplingInterval, "baseline", 0)
				endif
...
static Function DC_AddEpochsFromStimSetNote(panelTitle, channel, stimset, stimsetBegin, setLength, sweep, scale)
	...
	for(epochNr = 0; epochNr < epochCount; epochNr += 1)
		...
		if(flipping)
			...
			epochBegin = limit(epochEnd - duration[epochNr], stimsetBegin, Inf)
		else
			epochBegin = sweepOffset[epochNr] + stimsetBegin
			epochEnd = limit(epochBegin + duration[epochNr], -Inf, stimsetEnd)
		endif
		...
		DC_AddEpoch(panelTitle, channel, epochBegin, limit(epochEnd, -Inf, stimsetEnd), epSubName, 1)
		// Add Sub Sub Epochs
		if(epochType == EPOCH_TYPE_PULSE_TRAIN)
			...
			for(pulseNr = 0; pulseNr < numPulses; pulseNr += 1)
				if(flipping)
					...
					if(!pulseNr && subEpochEnd < epochEnd && subEpochEnd < stimsetEnd)
						DC_AddEpoch(panelTitle, channel, subEpochEnd, limit(epochEnd, -Inf, stimsetEnd), "baseline", 2)
					endif
				else
					...
					if(!pulseNr && subEpochBegin > epochBegin)
						DC_AddEpoch(panelTitle, channel, epochBegin, subEpochBegin, "baseline", 2)
					endif
				endif
				epSubSubName = ReplaceNumberByKey("Pulse", epSubName, pulseNr, STIMSETKEYNAME_SEP, EPOCHNAME_SEP)

				subEpochBegin = limit(subEpochBegin, stimsetBegin, Inf)
				subEpochEnd = limit(subEpochEnd, -Inf, stimsetEnd)
				DC_AddEpoch(panelTitle, channel, subEpochBegin, subEpochEnd, epSubSubName, 2)
			endfor
		else
...
static Function DC_SortEpochs(panelTitle)
	...
	for(channel = 0; channel < channelCnt; channel += 1)
		...
		if(epochCnt)
			...
			Redimension/N=(-1, -1, 0) epochChannel
			epochChannel[][%EndTime] = num2str(-1 * str2num(epochChannel[p][%EndTime]))
...
static Function DC_AddEpoch(panelTitle, channel, epBegin, epEnd, epName, level)
	string panelTitle
	variable channel
	...
	ASSERT(epBegin >= 0, "Epoch begin is < null")
	ASSERT(epEnd >= 0, "Epoch end is < null")
