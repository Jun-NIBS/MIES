...
static Function TestEpochsMonotony(e, DAChannel)
	...
	Make/FREE/N=(epochCnt) startT, endT, durT, marker, isOodDAQ
	...
	do
		...
		if(isOodDAQ[epochNr])
			...
			REQUIRE(startT[epochNr] >= 0 && endT[epochNr] <= endInt)
			marker[epochNr] = 1
		else
			for(i = 0; i < epochCnt; i += 1)
...
static Function TestEpochsGeneric(device)
	...
	CHECK_EQUAL_VAR(DimSize(epochs, COLS), 3)
	...
	for(i = 0; i < numEntries; i += 1)
		...
		Make/FREE/O/N=(DimSize(epochChannel, ROWS)) endT
